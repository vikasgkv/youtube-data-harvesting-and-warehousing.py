...
#api_key
Api_key = 'AIzaSyDKdF20jzmRsZQ5HOVWLzf-ej9Xe_1Zc0s'
...
a,b = st.tabs([" Retrieve Channel Data ", " Store Data In MongoDB  "])
c,d = st.tabs([" Retrieve Data From MongoDB  ", " Create tables In SQL"])
e,f = st.tabs(["Migrate channel information from MongoDB  to SQL",
     " Migrate video information from MongoDB  to SQL "])
g,h = st.tabs(["Migrate comment information from MongoDB to SQL",
     "channel analysis"])
...
with e:
    if st.button("Migrate channel information from MongoDB  to SQL"):
        ...
        mydb.commit()
        st.success("Data stored successfully in MongoDB and migrated to SQL data warehouse!")

with f:
    if st.button("Migrate video information from MongoDB to SQL"):
        migration_data = collection.find_one({'Channel_Name.Channel_Id': channel_id})
        if migration_data:
        ...
        st.success("Data stored successfully in MongoDB  and migrated to SQL data warehouse!")
with g:
    if st.button("Migrate comment information from MongoDB  to SQL"):
        migration_data = collection.find_one({'Channel_Name.Channel_Id': channel_id})
        if migration_data:
            for video_id, video_data in migration_data.items():
                if video_id != 'Channel_Name' and not isinstance(video_data, ObjectId):
